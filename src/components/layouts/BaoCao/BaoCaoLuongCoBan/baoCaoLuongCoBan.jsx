  ...
  const handleSearch = () => {
    ...
    if (!_.isEmpty(watch().month)) {
      if (!_.isEmpty(watch().department_name) && !_.isEmpty(watch().month)) {
        ...
        console.log("filter ", `odata/FixedSalarys?${filter}`);
        callApi(`odata/FixedSalarys?${filter}`, "GET").then((res) => {
  ...
  return (
    <div>
      <PageHeader
        ...
        title="Báo cáo bảng lương cơ bản"
      />
      <Row
        gutter={[16, 0]}
        className="toolBar"
            ...
            <table id="table" style={{ display: "none" }}>
              <thead>
                <tr>
                  ...
                  <th>Hệ số lương cơ bản</th>
                  <th>Phụ cấp thâm niên vượt khung</th>
                  <th>Phụ cấp chức vụ</th>
                  <th>Hệ số trách nhiệm công việc</th>
                  <th>Hệ số độc hại</th>
                  <th>Hệ số ưu đãi</th>
                ...
                {_.map(lstReport, (item, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td>{item.salary_coef}</td>
                      <td>{item.seniority_coef}</td>
                      <td>{item.position_coef}</td>
                      <td>{item.responsibility_coef}</td>
                      <td>{item.toxic_coef}</td>
                      <td>{item.favour_coef}</td>
...
BaoCaoLuongCoBan.propTypes = {};

export default BaoCaoLuongCoBan;
