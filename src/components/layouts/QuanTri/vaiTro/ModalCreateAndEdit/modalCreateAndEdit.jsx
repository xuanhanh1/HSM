...
function ModalCreateAndEdit(props) {
  ...
  const onSubmit = (data) => {
    // console.log(data);
    if (isLstPickMenu.length === 0) {
      openNotificationWithIcon("warning", "Vui lòng chọn quyền cho vai trò");
    } else {
      if (isStatus === 0) {
        //Thêm mới
        ...
        //Sửa
        console.log(objEdit);
        const newLstPickMenu = objEdit.MENUS.map((item) => {
          return { MENUID: item.id };
        });
        console.log([...newLstPickMenu, ...isLstPickMenu]);
        setResult(
          dispatch(
            editRoles({
              ...data,
              LstMenus: isLstPickMenu,
            })
          )
        );
      }
